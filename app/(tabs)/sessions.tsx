...
export default function SessionView() {
  ...
  const { sessionsList, setSelectedSession, addSession, removeSession } =
    useDatabase();
  ...
  return (
    <PaddedSafeAreaView>
      ...
      <Pressable onPress={() => createNewSession()} style={styles.footerButton}>
        <Text>Create New Session</Text>
      </Pressable>
      <FlatList
        data={sessionsList}
        keyExtractor={(item) => (item.id ? item.id.toString() : "idk")}
        renderItem={({ item }) => (
          <View>
            <Text>{item.name}</Text>
            <Text>{item.date}</Text>
